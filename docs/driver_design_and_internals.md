...
基于clang11版本文档翻译  
<https://releases.llvm.org/15.0.0/tools/clang/docs/DriverInternals.html>
...
<p align="center">
<img src="https://releases.llvm.org/15.0.0/tools/clang/docs/_images/DriverArchitecture.png">
</p>
...
为了尽量贴近gcc，clang驱动器目前允许工具链自己执行对参数的解析（结果放到新的ArgList数据结构中）。虽然这让clang驱动器贴近gcc了，驱动器的操作也变得很难懂了（因为`Tool`实例不是直接操作用户输入的参数，而是会处理工具自己解析后的）

比如，在Darwin上，`-gfull`会被翻译成2个分开的参数，`-g`和`-fno-eliminate-unused-debug-symbols`。那么在`Tool`上编写处理`-gfull`的逻辑就很麻烦，因为`Tool`的参数翻译是在参数已经经过一次翻译之后执行的。  

长期的目标是删除这类工具链相关的翻译动作，并且修改工具的逻辑使其可以处理原本的未翻译的参数。  

## 未使用的参数警告

驱动器的操作方式是将所有参数传给`Tool`，但是让`Tool`可以选择实际需要的部分。这个架构的一个缺点是：如果用户拼错了选项，或者弄混了选项的使用，一些用户需要的参数可能处理时没有被用到。这个问题在使用clang作为编译器时更严重，因为clang对gcc的选项不是完全兼容的，这个时候就希望能告知用户哪些是被真正用到的。  

为了支持这个能力，驱动器会为每个参数维护一个记录其是否被使用的bit。这个bit一般不用手动标记，ArgList的访问接口会自动设置之。  

编译成功（没有报错）时，驱动器会检查这些bit并对那些没有被访问到的参数抛出“未使用的参数”的警告。这个处理策略略微保守（可能这个参数没有按照用户期望的方式被使用），不过仍然可以覆盖大部分的明显有问题的情况。  

# 和GCC驱动概念的关系

本节对clang驱动和gcc驱动的一些关系做了简略的总结，方便那些对gcc驱动比较熟悉的用户。
- Driver Driver
  
Driver driver整个集成到了clang驱动中。这个驱动只是在管道阶段简单地构造了附加的Action绑定到具体的架构上。工具链相关的参数翻译只负责处理`-Xarch_`。  

需要注意的是，这样做的目的是让`-Xarch_`参数不要影响到编译过程自身（比如，不能将`-S`作为`-Xarch_`的参数）。驱动器会尝试禁止这样的操作，而且实践中也没有什么像样的理由来说明为何要这样随便使用`-Xarch_`。

clang驱动相比而言好的一点是，效率更高，并且在进行通用的构建时需要更少的额外工作。而且，可以提供更好的错误报告内容，在与UI的相互搭配上也表现更好。

- 规范

Clang的驱动没有很直白的对应的规范内容。规范需要的大部分的功能都在工具相关的参数翻译部分。控制编译流程的那一部分规范则作为管道阶段的一部分存在。

- 工具链
  
gcc驱动器对工具链没有特别直接的理解。每个gcc的二进制文件只是很粗粒度地对应到一个单独的ToolChain中的信息。  
Clang驱动器期望是可移植的，并且可以支持很复杂的编译环境。所有的平台和工具链相关的代码被设计成隐藏在抽象或者良好定义的接口之下（比如，这个平台是否可以作为driver driver使用）。
